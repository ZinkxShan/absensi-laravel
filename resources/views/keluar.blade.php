...
<meta name="csrf-token" content="{{ csrf_token() }}">
  ...
  <title>Absensi — Scan Keluar</title>
  <link
...
<body>
  ...
  <main>
    ...
    <div class="mode-badge">
      <div class="mode-dot"></div> Absen Keluar
    </div>

    <div class="scan-card">
      <div class="scan-label">Scan QR Code — Keluar</div>
    ...
    <div class="recent-card">
      <div class="recent-title">Baru Saja Keluar</div>
      <ul class="recent-list" id="recent-list">
        <li class="empty-state">Belum ada absensi keluar hari ini</li>
      </ul>
    </div>
  </main>
  ...
  <script>
    ...
    inp.value = kode;
    try {
        const res = await fetch('/api/scan/keluar', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': CSRF,   // ← tambahkan ini
            },
    ...
    function showResult(data) {
      ...
      document.getElementById('result-icon').textContent = data.status === 'berhasil' ? '✓' : data.status === 'sudah' ? '⚠' : '✗';
      document.getElementById('result-nama').textContent = data.nama || data.pesan || 'Error';
      let detail = '';
      if (data.kelas) {
        detail = `Kelas ${data.kelas}`;
        if (data.status === 'berhasil' && data.waktu_masuk) detail += ` · Masuk ${data.waktu_masuk.slice(0, 5)}`;
        if (data.status === 'sudah') detail += ' · Sudah absen keluar';
        if (data.status === 'error' && data.kelas) detail += ' · Belum absen masuk!';
      } else {
        detail = data.pesan || '';
      }
      document.getElementById('result-kelas').textContent = detail;
    ...
    function addRecent(data) {
      ...
      <li class="recent-item">
        ...
        <div style="flex:1">
          ...
          <div class="masuk-info">Masuk ${d.waktu_masuk ? d.waktu_masuk.slice(0, 5) : '-'}</div>
        </div>
